...
def admin_dashboard_view(request):
    ...
    data = {
        ...
        'total_teacher': Teacher.objects.filter(status=True).count(),
        'total_course': Course.objects.count(),
        'total_question': Question.objects.count(),
    }

    # Course enrollment data (using Results model)
    courses = Course.objects.all()
    course_data = []
    for course in courses:
        # Assuming you have a Result model linking students and courses
        enrolled_students = Result.objects.filter(exam=course).values('student').distinct().count()
        course_data.append({
    ...
    question_data = Course.objects.annotate(total_questions=Count('question')).values('course_name', 'total_questions')

    # Recent activity
    recent_students = Student.objects.order_by('-user__date_joined')[:5]
    recent_teachers = Teacher.objects.order_by('-user__date_joined')[:5]

    data.update({
        'course_data': json.dumps(list(courses.values('course_name'))),
        'enrollment_data': json.dumps([item['enrollments'] for item in course_data]),
        'question_labels': json.dumps([item['course_name'] for item in question_data]),
        'question_values': json.dumps([item['total_questions'] for item in question_data]),
        'recent_students': recent_students,
        'recent_teachers': recent_teachers,
    })

    return render(request, 'exam/admin_dashboard.html', context=data)
@login_required(login_url='adminlogin')
